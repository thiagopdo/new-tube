...
/**
 * Video management procedures for the tRPC router
 *
 * Procedures:
 * - getOne: Get a single video by ID with associated metadata
 *   - Input: { id: string }
 *   - Returns video details, user info, view/reaction counts
 *   - Accessible to all users (baseProcedure)
 *   - Includes viewer-specific reaction data if authenticated
 *
 ...
 *
 * - delete: Delete a video and associated assets
 *   - Input: { id: string }
 *   - Protected route requiring authentication
 *   - Validates video ownership
 *   - Cleans up Mux assets and UploadThing files
 *
 * Relations:
 * - Joins with users table for video owner details
 * - Aggregates videoViews for view count
 * - Aggregates videoReactions for like/dislike counts
...
export const videosRouter = createTRPCRouter({
  ...
  //REVALIDATE VIDEO ON MUX UPLOAD CHANGE
  ...
  create: protectedProcedure.mutation(async ({ ctx }) => {
    ...
    const upload = await mux.video.uploads.create({
      ...
      cors_origin: "*", //TODO: set to url in production
    });
